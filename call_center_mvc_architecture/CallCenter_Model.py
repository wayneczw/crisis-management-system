...
def get_db():
    ...
    db = getattr(g, '_database', None)  # g is a global/object that would last for only 1 request/response cycle [data in g does not persist, unlike a database]
    if db is None:   
        db = g._database = sqlite3.connect(DATABASE)   # sqlite3.connect(DATABASE) will create the Database if that Database does not yet exist.  
    return db
...
def __insert_incident(name, mobile_number, location, assistance_required, description, priority_injuries, priority_dangers, priority_help, report_status, 
                            report_time):
...
def insert_report(name, mobile_number, location, assistance_required, description, priority_injuries, priority_dangers, priority_help, report_status):
    ...
    incident_report_id = __insert_incident(name, mobile_number, location, assistance_required, description, priority_injuries, priority_dangers, priority_help, report_status,
                            report_time)
...
def update_report(id_of_incident_report, caller_name, caller_mobile_number , caller_location, type_of_assistance, description , priority_for_severity_of_injuries, priority_for_impending_dangers, priority_for_presence_of_nearby_help, report_status, is_first_such_incident):
...
def retrieve_all_incident_reports():
    ...
    with app.app_context():  # Within the Application Context:
        ...
        for report in all_incident_reports:
            ...
            report[1] = report[1][:-7] # Remove the last 7 chars of the timestamp for "First reported time" as they are too precise.
