...
app = Flask(__name__, template_folder = "CallCenter_Views")
app.secret_key = "a2nbs81sSJl8d"  # Set Encryption key to encrypt the Data sent by client during a Session [These Data is stored in the encrypted form]
...
@app.route("/update_incident_report_page", methods = ['GET', 'POST'])
def update_incident_report():
    ...
    return render_template('callcenter_ui_update_incident_report.html', columns=['ID', 'First reported time', 'Name', 'Mobile no.' , 'Location', 'Assistance requested', 'Description' , 'Priority on injuries', 'Priority on dangers', 'Priority on help', 'Report status', 'Is first such incident'], items=all_incident_reports)
...
@app.route('/row_detail_for_update/<rowData>', methods = ['GET', 'POST'])
...
@app.route("/delete_incident_report_page", methods = ['GET', 'POST'])
def delete_incident_report():
    all_incident_reports = retrieve_all_incident_reports()
    if request.method == 'POST':
        return redirect('/')
    return render_template('callcenter_ui_delete_incident_report.html', columns=['ID', 'First reported time', 'Name', 'Mobile no.' , 'Location', 'Assistance requested', 'Description' , 'Priority on injuries', 'Priority on dangers', 'Priority on help', 'Report status', 'Is first such incident'], items=all_incident_reports)
...
@app.route('/row_detail_for_delete/<rowData>', methods = ['GET', 'POST'])
...
@app.route("/incident_report_delete_completion_page", methods = ['GET', 'POST'])
...
@app.route("/incident_report_sent_completion_page", methods = ['GET', 'POST'])
...
def submit_new_incident_report():
    ...
    if request.method == 'POST':
        ...
        return redirect('/incident_report_sent_completion_page')
